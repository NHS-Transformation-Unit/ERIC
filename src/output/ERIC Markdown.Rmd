...
This report uses site codes to shorten names and make visuals easier to read. There are a total of `r Orgcount` sites included for your organisation. Each can be found in the map of the "Site tenure" tab, and in the table in the "Organisation sites" tab.

### Site tenure
This map shows the location of all the sites included in the ERIC returns for your organisation, colour coded by type of tenure.
```{r map_output}
...
### Organisation sites
This is a reference table showing the sites that are included in this report, and the code used to refer to them in the visuals.
```{r Sites}
RCode <- params$organisation
...
## Estates Backlog {.tabset .tabset-fade}
***
...
### Total estate cost
The below chart shows the total cost to reach condition B, by risk level.

```{r TrustB_backlog_plot}
RCode <- params$organisation
TrustB_backlog_plot(RCode)
```

### Estate cost per site
The below chart shows the total cost for each site to reach condition B, broken down by the risk level. Each bar has the highest risk at the top and the lowest risk at the bottom.

```{r TrustA_backlog_plot}
RCode <- params$organisation
TrustA_backlog_plot(RCode)
```
